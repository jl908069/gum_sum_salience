...
def main():
    ...
    alignments = align(all_entities_from_tsv, list(summaries.values()), all_mentions, doc_sp_texts, n_summaries=1 , component=args.alignment_component, partition=args.partition)
    ...
    json.dump(pred, open(os.path.join(f"./data/output/alignment/{args.alignment_component}", f"pred_{args.model_name.split('/')[-1]}.json"), "w"), indent=4) if os.makedirs(f"./data/output/alignment/{args.alignment_component}", exist_ok=True) is None else None
